...
def send_message(resp):
    ...
    for chat in arr:
        ...
        if username in db_info:
            ...
            if os.getenv('TWITCH_IGNORE') not in message and 'esportsbet' not in message.lower() and '100 usdt' not in message not in message.lower() and 'lichess' not in message.lower():
                hook = SyncWebhook.partial(webhookid, webhooktoken)
